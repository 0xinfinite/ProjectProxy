Shader "Hidden/Scene Depth Shader"
{
    Properties
    {
...
//#endif
            ...
            VertexPositionInputs GetVertexPositionInputsOverrided(float4 positionOS, float3 offset)
            {
                VertexPositionInputs input;

                input.positionWS = TransformObjectToWorld(positionOS.xyz) + offset;
            ...
            CBUFFER_END
                ...
                UNITY_DOTS_INSTANCED_PROP(float3, _PrevPosition)
...
#define _PrevPosition             UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(float3 , _PrevPosition)
#endif
...
#include "../ShaderLibrary/Projection.hlsl"
                ...
                struct Varyings
                {
                    ...
                    float3 objWorldPos : TEXCOORD7;
                    float3 worldSpaceViewDir : TEXCOORD8;
                    float speed : TEXCOORD9;
                    float3 velocityNormalized : TEXCOORD10;
                    float isMovable : TEXCOORD11;
               ...
               /* TEXTURE2D(_DepthTex);
                ...
                Varyings LitPassVertex(Attributes input)
                {
                    ...
                    float3 objWorldPos = GetAbsolutePositionWS(UNITY_MATRIX_M._m03_m13_m23);
                    float3 velocity = objWorldPos - _PrevPosition;
                    float magnitude = length(velocity);
                    output.velocityNormalized = velocity / max(magnitude, 0.0001f);

                    output.isMovable = saturate(abs(length(_PrevPosition)));
                    output.speed = Remap(magnitude, 0, unity_DeltaTime.x, 0, 1);
                    //float distanceWithCamera = length( objWorldPos - _WorldSpaceCameraPos.xyz);
                    //input.positionOS.xyz += (input.positionOS.xyz / max(length(input.positionOS.xyz),0.0001f)) * saturate(output.speed)*output.isMovable * distanceWithCamera*0.1f;

                    //float3 worldSpaceCameraVector = objWorldPos - _WorldSpaceCameraPos.xyz;
                    //float3 worldSpaceCameraDir = worldSpaceCameraVector / max(length(worldSpaceCameraVector), 0.0001f);

                    VertexPositionInputs vertexInput = GetVertexPositionInputs(input.positionOS);//GetVertexPositionInputsOverrided(input.positionOS, worldSpaceCameraDir * saturate(output.speed) * output.isMovable * distanceWithCamera * 0.1f);

                    output.positionWS = vertexInput.positionWS;


                    float3 worldSpaceViewVector = _WorldSpaceCameraPos.xyz - vertexInput.positionWS;
                    float viewDirMagnitude = length(worldSpaceViewVector);
                    output.worldSpaceViewDir = worldSpaceViewVector / max(viewDirMagnitude, 0.0001f);


                    output.positionCS = vertexInput.positionCS;
                    //output.positionCS.xy *= 5;
                ...
                )
                {
                  ...
                  // outColor.b= 0;//remap(outColor.b,-1,1,0,1);
                   //
                    ...
                    float hot = dot(input.velocityNormalized, input.worldSpaceViewDir);
                    float cold = dot(input.velocityNormalized, input.worldSpaceViewDir * -1);


                    hot = Remap(hot, 0.3f, 1, 0, 1);
                    hot = saturate(hot);
                    hot *= input.isMovable;
                    hot *= input.speed;
                    cold = Remap(cold, 0.3f, 1, 0, 1);
                    cold = saturate(cold);
                    cold *= input.isMovable;
                    cold *= input.speed;
                    ...
                    outColor = float4(hot, 1-z01, cold, 1);
                    //return;
